...
public class StoreWritingTests
{
    ...
    {
        _eventStore.Append(Some.EventType, Some.EventData);
        _eventStore.Append(Some.EventType, Some.EventData, 0);
    ...
    public void storing_an_event_with_query()
    {
        ...
        _eventStore.Append(Some.EventType, Some.EventData); //event that we don't care about
        _eventStore.Append(Some.EventType, Some.EventData); //event that we don't care about
        ...
        Should.NotThrow(() =>
        {
            _eventStore.Append(Some.EventType, Some.EventData, [identifier], lastKnownPosition, query);
        });
    }
